...
Since the guest account could authenticate without a password, I used netexec again to enumerate the SMB shares available to this user.
...
To reduce the massive list generated by the `RID cycling`, I wrote a small Python script to parse `nxc-cmd.txt` and extract only entries that match the structure of valid domain usernames. The script filtered out noise, machine accounts, and malformed lines, producing a clean `usernames.txt` file.
...
The presence of `Users` with readable permissions made it a promising target for further exploration.
...
With this new account I once again listed the shares and discovered that in addition to the ones I could aready read with the previous user account, backup was now readable too.
...
With the newly cracked service account credentials, I enumerated the SMB shares again to see whether this account had access to anything the previous user couldn’t.

```bash
smbclient //$TARGET/backup -U file_svc
```

In addition to the shares already accessible before, this account also had read access to the `backup` share.

![get backup_extract.txt](media/22.png)

Digging into the Backup share paid off.     
The backup extract conveniently contained machine account data from the domain, including their corresponding password hashes.
...
Now with valid credentials for `FileServer$` account, I enumerated its SMB shares to determine what level of access I now had.
...
add Privilege Escalation and Post‑Exploitation sections.
